...
QuaRot Hadamard rotation (`quantize_quarot` is the rotated variant — lower quantization error,
extra preprocessing step; see `docs/q4-quantization.md`).
...
./target/release/quantize_q4 \
  ...
  --output-dir /private/tmp/lattice-602-scratch/qwen3.5-0.8b-q4
```
...
Index written: /private/tmp/lattice-602-scratch/qwen3.5-0.8b-q4/quantize_index.json
...
- **Missing `config.json` silently loads the wrong architecture preset.** With no `config.json`
  ...
  ```bash
  cp ~/.lattice/models/qwen3.5-0.8b/config.json /private/tmp/lattice-602-scratch/qwen3.5-0.8b-q4/
  ```
...
- A model directory: either a BF16/F16 safetensors checkpoint (`model.safetensors[.index.json]`
  - `config.json` + `tokenizer.json`) or a Q4 directory (`*.q4`/`*.f16` files + `quantize_index.json`,
    ideally also `config.json` + `tokenizer.json` — see the `quantize_q4` failure modes above for
...
| `--model-dir <DIR>`                           | —                                                            | Takes precedence over `--model` if both given                                                                                         |
...
| `--prompt <TEXT>`                             | —                                                            | Required for one-shot `--json` mode; ignored in REPL/`--serve` mode                                                                   |
| `--max-tokens <N>`                            | 512                                                          |                                                                                                                                       |
| `--seed <N>`                                  | none (random)                                                |                                                                                                                                       |
| `--temperature <F>`                           | 0.7                                                          |                                                                                                                                       |
| `--top-k <N>`                                 | 50                                                           |                                                                                                                                       |
...
```bash
./target/release/chat_metal --model ~/.lattice/models/qwen3.5-0.8b \
...
@@lattice {"ev":"gen_token","token":"1","token_id":16,"done":false}
...
...
./target/release/chat_metal \
  --model /private/tmp/lattice-602-scratch/qwen3.5-0.8b-q4 \
  --tokenizer-dir ~/.lattice/models/qwen3.5-0.8b \
  --prompt "Hi, tell me a fact about the ocean in one sentence." --max-tokens 24 --json
```

(same event stream shape as above, `21.9`→`21.9` tok/s range; confirms `--tokenizer-dir` is the
correct override).
...
printf '{"prompt":"Say hi in one word.","max_tokens":6}\n' \
  | ./target/release/chat_metal --model ~/.lattice/models/qwen3.5-0.8b --json --serve
```

```
[chat_metal] Loading bf16 model from /Users/lion/.lattice/models/qwen3.5-0.8b...
...
— the app can show an honest "loaded" state without sending a generation request first. The
...
printf 'Say hi in one word.\n\n' \
  | ./target/release/chat_metal --model ~/.lattice/models/qwen3.5-0.8b-q4 \
      --tokenizer-dir ~/.lattice/models/qwen3.5-0.8b --max-tokens 16
```

```
[chat_metal] Detected Q4 model directory: /Users/lion/.lattice/models/qwen3.5-0.8b-q4
[chat_metal] Loading Q4 model...
...
- A plain-text corpus file (no default corpus ships in the repo — you must supply one).
...
(a few paragraphs of plain prose, ~330 words):

```bash
LATTICE_MODEL_DIR=~/.lattice/models/qwen3.5-0.8b \
PPL_TOKENS=256 \
CORPUS=/private/tmp/lattice-602-scratch/sample_corpus.txt \
  ./target/release/ppl_metal
```

```
[ppl_metal] loading /Users/lion/.lattice/models/qwen3.5-0.8b
...
- Pointing `LATTICE_MODEL_DIR` at a Q4 directory fails at `Qwen35Model::from_safetensors` — this
  binary has no Q4 loading path; use `eval_perplexity --q4-dir` for Q4 perplexity instead (see
  `docs/q4-quantization.md`).
...
- For the `DECODE_BENCH` half: a Q4 model directory that has **both** `config.json` and
  ...
  default (see the `quantize_q4` section above) — copy them in first.
...
LATTICE_MODEL_DIR=/private/tmp/lattice-602-scratch/qwen3.5-0.8b-q4 \
...
...
[bench_lora_mixture] loading model from /private/tmp/lattice-602-scratch/qwen3.5-0.8b-q4
...
- **`GDN layer` crash on the `DECODE_BENCH` half — confirmed bug, filed as
  ...
  (`full_attention_interval=4`) is `full_attention` (layers 3, 7, 11, 15, 19, 23 — 24 layers
