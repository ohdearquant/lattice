...
> _fraction estimate_ (f = 2.5–7.8%) is an external computation, not a measurement from this
...
- **First-class exact state object + observability layer** — CONFIRMED-BY-ANALYSIS as
  prerequisites for quantization, routing, branch/rollback, and failure detection. _This repo
  already contains both in partial form (S5 buffers, S6 counters); the survey confirms the
  direction, not a greenfield build._
- **State compression for single-request decode SPEED** — REFUTED by roofline. With state
  ...
  survey's f is _independently derived_, not a repo measurement (S7), but the bound is robust
  across the whole plausible single-digit-to-low-double-digit range, and S2's context-invariance
  means f _shrinks_ as context grows. Compression is not a decode-speed lever.
...
3. **P3 — Formalize observability modes (Off / Canary / Debug) on the P2 object.** The counters
   ...
   token, reject it). Prerequisite for downstream precision/routing experiments. _Build, after P2._
...
The B=128 device-scratch + `simdgroup_matrix` variant is the _sole_ untried chunk-scan sub-lever,
...
- **Risk.** If #491 measures f in the low double digits at short context / high batch, the
  ...
  ≤ 8% with r = 4, so the closure holds as a _priority_ call even then; only the storage-compression
