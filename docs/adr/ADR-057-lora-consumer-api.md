...
- **Over-declared `d_out`** — the shape check `output.len() >= d_out` fails, `apply_lora` no-ops, and the projection row is left exactly as the base weights produced it. The score is bit-for-bit the unadapted score. No panic, no partial write.
- **Under-declared `d_out`** — the same check *passes*, because it is an inequality, and the accumulate loop writes `output[..d_out]`. The first `d_out` elements receive an update computed for a geometry the model does not have, the remainder are untouched, and a finite in-range score comes back. This is a silent partial write, and it is the exposure the fail-open default actually leaves open.

We accept it for 0.7.0 on the ground that it is bounded in blast radius rather than absent: it cannot slice out of bounds, cannot panic, and cannot escape the hooked projection. It is not the same thing as the guarantee `score_with_hook` reads as offering, which is why the revisit below is scheduled rather than optional.
