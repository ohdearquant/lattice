...
What remains for the revisit is the original question and only that one: a hook that overrides nothing is still trusted, so validation is opt-in. The exposure that leaves is now a *dropped* adapter, not a corrupted projection.
