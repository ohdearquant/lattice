...
Issue #439 (PR #444) shipped the governed manifest schema and fail-closed loader, but an audit (#610) found three gaps against the original #439 field-set intent. This section records the closing decisions.

**1. `ManifestEntry` field set.** #439 specified `{name, owner, uri, base_model_rev, tokenizer_rev, integrity_sha256, dtype, approved}`. `owner: String` and `dtype: String` were added (both free-form provenance strings, not enforced against tensor bytes at load time). `approved: bool` was **not** added as a separate field: `status: AdapterStatus` (`Approved | Quarantined | Revoked`) is a strict superset — `status == Approved` **is** `approved == true`, and the `Quarantined`/`Revoked` variants already have distinct fail-closed error messages the loader and its tests depend on. Adding a redundant `approved: bool` alongside `status` would create two sources of truth for the same governance decision with no defined precedence when they disagree. `AdapterStatus::as_str()` was added for callers that need a plain string form (e.g. embedding into a safetensors header) without re-deriving it from serde.

**2. Safetensors header embedding.** `save_peft_safetensors` gained an `Option<&AdapterGovernance>` parameter (`AdapterGovernance { name, owner, base_model_rev, tokenizer_rev, dtype, status }`, all plain strings) embedded under `gov_name` / `gov_owner` / `gov_base_model_rev` / `gov_tokenizer_rev` / `gov_dtype` / `gov_status` metadata keys when supplied. `integrity_sha256` is **deliberately excluded**: it is a SHA-256 hash of the complete safetensors file, header included; a header field cannot coherently hold a hash of "this file including this field" (writing the hash changes the header bytes, which changes the true hash — there is no fixed point short of a cryptographic preimage). The manifest remains the sole authority for whole-file integrity (Check 4). A reader, `read_peft_header_governance`, round-trips the six fields back out for tooling/tests; it is `pub(crate)` for now (no external consumer requested one).

**3. Revision enforcement (the substantive gap).** `load_adapters_from_manifest` gained an eleventh check and a new `running: Option<&RunningRevisions<'_>>` parameter (`RunningRevisions { base_model_rev, tokenizer_rev, allow_mismatch }`), mirroring the existing `model_config: Option<&Qwen35Config>` idiom used for dimension validation. `None` skips enforcement entirely (e.g. offline manifest validation with no live model context). `Some` compares both fields with plain string equality — the literal `"none"` sentinel some entries use for "untracked" is **not** treated as a wildcard, to avoid a magic-string bypass in an otherwise-strict fail-closed path. Default is strict (`RunningRevisions::strict`, `allow_mismatch: false`): any mismatch rejects the adapter. `RunningRevisions::permissive` (`allow_mismatch: true`) lets a mismatch through for controlled migrations, and the override is recorded — never silent — via a new `LoadedAdapter::rev_mismatch_overridden: bool` field. The check is appended as "Check 11" after the existing ten rather than renumbering them, to keep the change additive per #610's own guidance (and to minimize merge-conflict surface with #615's planned shared-descriptor extraction, which is out of scope here).
