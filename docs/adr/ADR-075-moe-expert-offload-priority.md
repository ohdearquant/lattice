...
> **NEEDS-EXPERIMENT, reframed**: the ADR-075 question as commissioned — _rank the open MoE
> expert-offload levers_ — presupposes a working, measured, resident MoE serving path whose next
...
1. **Nothing measured on this engine touches MoE.** Lattice ships exactly one MoE preset
   ...
   512). The large model it actually _serves and has measured_ (18 GB Q4, ~4 tok/s decode) is
...
2. **The Metal MoE dispatch is real and wired, but unmeasured and f16-only.** ADR-053's per-expert
   ...
   f16**: Q4/QuaRot, Q8-CPU, and Q8-NEON _all three_ explicitly refuse MoE configs.
3. **The resident footprint is 60 GiB (f16), not 15 GiB, and the loader can't reach it anyway.**
   ...
   storage/bandwidth play, not a compute one — but here the prior question is whether the _resident_
   path works at all.
...
| E2  | **The MoE config is the struct `Default`** (`Qwen35Config::default() → qwen36_35b_a3b()`), yet no CLI/loader default selects it — the dense defaults are path-specific: Q4 chat/serve fall back to `qwen36_27b`, safetensors loading falls back to `qwen35_2b`, some benches to `qwen35_0_8b`. Reachable only by constructing the preset in Rust.                                                                                  | source-read                         | `c9ae21a9:bin/lattice.rs:1061` (safetensors→2b), `:1076` (Q4→27b); `qwen35_config.rs:154-158`; `model/qwen35/model.rs:37-42` (from_safetensors→2b) |
...
| E12 | **Prior MoE robustness work has landed**: MoE-config `.expect()` panic → structured error (#209 CLOSED), Q8-on-MoE panic → `UnsupportedModel` (#385 CLOSED), router NaN fail-closed (unit-tested). The CPU `moe_ffn_step` router is well-tested; only the _Metal dispatch_ and _loadability_ are unmeasured.                                                                                                                       | gh-verified + unit-test-pinned      | `gh issue view 209/385` CLOSED; `model/qwen35/moe.rs:54-89,247-745`                                                                                |
...
| f16 (current Metal GPU buffers, E6)    |      6.0 MiB |                 1.5 GiB |        **60 GiB** | happens today, _after_ the 120 GiB f32 load               |
...
target _once a working resident path exists_ — but at **f16 (60 GiB)**, not the Q4 (15 GiB) ADR-053
assumed.

## Amendment to ADR-053 (source-derived correction, recorded at priority level)

ADR-053 §"Expert weight memory" states, verbatim: _"At Q4 with the current config dimensions ... Total
routed expert weights across 40 layers: ~384 MiB × 40 ≈ **15 GiB**"_ (`ADR-053:48-51`), and its R2
risk analysis reasons from _"~15 GiB for routed expert weights ... borderline on 32 GB"_
...
- **Wrong model family**: its primary target throughout is DeepSeek-V4-Flash-DSpark (284B/13B-active,
  ...
  "moderate model" comparison uses a **1.5 MiB/expert** figure that matches lattice's _aspirational
  Q4_ size, not its _actual f16_ 6.0 MiB (4× larger). Its "90–95% hit-rate workable" conclusion must
  ...
  not refuted**.
- **One convergent, useful conclusion**: the survey independently reaches _"the first prototype should
  be an in-memory MoE implementation on a model that already fits, with exact routing parity and
  throughput measurements, before any disk streaming is added"_ — which matches this engine's
...
_resident_ path works and what it costs — which nobody has measured (E4/E5). This is **not** the
"REFUTED — wrong architecture" honest-negative of the KV-quant/MTP recons (MoE _does_ exist and _is_
...
- **The ADR-053 memory correction IS rankable now** (§ amendment) — it is source-derived, not gated,
  ...
  today, so the _first_ work is a loadable, measured resident path, not offload.
...
optimization _inside_ `from_safetensors` was not ruled out by source-read alone). Instrument an actual
...
`moe.rs:437-507` already construct) through the _real_ `MetalQwen35Engine` (not the CPU-router-only
...
required-hit-rate target. _Then_ #682's LRU/mmap design becomes decidable.
...
prerequisite for ranking _any_ MoE memory lever. StorageModePrivate #179 is an adjacent Metal
...
- **Cost.** Step 1 needs a synthetic-MoE dispatch bench harness built (none exists — E5). Step 0 needs
  ...
  _any_ MoE work, not just offload.
