...
| `e2e-parity / embedding drift baseline`                                                                    | correctness        | Same e2e workflow trigger; expensive job runs only when `engine=true`; `embed-drift --enforce --json`                                                                                                                                                                           | gated - re-embeds frozen corpus texts and fails closed on drift or missing weights, so this is correctness rather than performance.                                                                    |
