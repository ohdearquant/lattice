...
| `e2e-parity / parity-gate required aggregator` | correctness | Same e2e workflow trigger; `if: always()`; always reports; mirrors parity and embed-drift when `engine=true`; passes when no engine surface changed | gated - this is the required context shape that avoids path-filtered required checks getting stuck while still blocking changed engine PRs on failed parity or drift. |
