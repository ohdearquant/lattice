...
- **G3 — EWC anchor-pullback as a live anti-forgetting mode.** `penalty_gradient`/`ewc_lambda` is
  inert by design in v1 (Fisher-weighted damping is the shipped mechanism). **Re-entry:** when
  multi-task router refits show measurable forgetting of earlier tasks under Fisher-weighted
  damping alone, wire `ewc_lambda` live and A/B it against the damping-only path, with the existing
