...
> pipeline — is already shipped, so this ADR ranks the _open_ levers rather than proposing a
...
- **Offline-only rotation as a production quality path — REFUTED**, and this _matches_ the measured
...
Rank the open weight-quant levers by _measured decode leverage × quality risk_, leading with the
...
1. **P1 — MLP-only W3 (#420).** Highest leverage: decode is weight-bandwidth-bound and MLP GEMMs are
   ...
   **independent of the QuaRot net-negative** — it proceeds on the shipping unrotated-asymmetric-Q4
   ...
   golden (W5), plus `make bench-compare` for the decode-time claim. _Build._
2. **P2 — Online R3/R4 Hadamard rotation (#703).** This lever _decides whether QuaRot is worth
   shipping at all_: offline v0 is a measured net-negative (W4), so the shipped rotation
   ...
   **CPU/f16 rotation-equivalence first** (mean-abs ≤ 5e-4 vs unrotated) _before_ any Metal kernel,
   ...
   _Build, gated on the equivalence check._
3. **P3 — FP8 (E4M3) / FP4 (MXFP4/NVFP4) dequant-on-read loaders (#684/#683).** Lowest research risk
   ...
   (e.g. the DeepSeek-V4 line per the issue bodies), not speculatively. _Build, demand-gated._
...
**What was wrong.** The original Decision ranked MLP-only W3 (#420) as P1 _Build_, and both the
...
`weights/q3_weights.rs`, specified by Amendment 1) has the _same quantization semantics_ as the
...
**Reopen conditions.** Any future W3 attempt must name a _quality mechanism that changes the
error distribution_ — calibration-based quantization (GPTQ/AWQ-family), a proven online rotation
