...
"Adaptive reasoning" here means controlling _how much_ reasoning/thinking compute the model spends
...
- **"MTP draft-head disagreement is a free uncertainty signal" — NEEDS-EXPERIMENT, low confidence.**
  Converges on #496 _and_ carries the same saturation risk: ADR-074's MTP finding (~4.65% free-form
...
family has zero local-measured data, gate it behind a _defined_ minimal experiment rather than
