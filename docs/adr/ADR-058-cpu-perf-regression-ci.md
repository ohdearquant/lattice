...
The end-to-end decode bench (`bench_decode_harness.py`, profile `apples_to_apples_q8`) catches macro regressions but is too noisy to attribute them: on a busy M2 Max with 5–10 concurrent GPU-bench processes, lattice decode throughput swings by 25% (issue #77). We cannot use noisy end-to-end numbers as a merge gate.
