...
| 3c   | series in progress (5 sub-PRs) | `quantize_quarot` binary tying 3a + 3b + the rotation math from PR #18. Owns the step-3c contract spelled out under §"Step 3c contract" below (rotation absorption in f64, RMSNorm fusion, lm_head materialization, forward-equivalence refuse-on-fail, Q4 bridge decision). Sub-PRs: 3c-1 PR #25 (merged) Q4 bridge entry points; 3c-2 PR #26 (merged) rotation + RMSNorm fusion pipeline (library); 3c-3 PR #27 (merged) lm_head materialization + `tie_word_embeddings` config flip; **3c-4 this PR** forward-equivalence harness + refuse-on-fail; 3c-5 (next) `bin/quantize_quarot` CLI wiring. |
...
- Tied-embedding untying when present. Step 3c-3 landed in PR #27 (`crate::quant::quarot::lm_head`): when `cfg.tie_word_embeddings`, the converter clones `model.language_model.embed_tokens.weight` into a separate `lm_head.weight` working-set entry so the two matrices can carry different post-QuaRot transforms (`embed_tokens` absorbs only the residual rotation; `lm_head` additionally absorbs the shifted `(1 + g_final)` final-RMSNorm scale before the rotation, because `diag(1 + g_final)` does not commute with the Hadamard rotation — see `plan.rs` §"Tied embeddings: only one correct path"). Untied configs validate `lm_head.weight` shape `[vocab_size, hidden_size]` at the converter boundary (`qwen_required_tensor_names` is name-only and `fuse_rmsnorms` only checks columns).
- `tie_word_embeddings=false` config flip in the output. **Two-flag gotcha**: HF Qwen3.5 `config.json` carries `tie_word_embeddings` in two places — nested under `text_config` and at the top level — and `Qwen35Config::from_config_json_str` gives the **top-level** value precedence. Flipping only the typed in-memory `cfg.tie_word_embeddings` is therefore not sufficient; the converter MUST mutate the on-disk JSON. Step 3c-3 provides `untie_word_embeddings_in_config_json(json) -> Result<String>` which sets the top-level flag to `false` and updates the nested flag if present, with a real-fixture round-trip test asserting that a re-parsed config evaluates to `!tie_word_embeddings`. The in-memory `untie_word_embeddings_in_cfg(cfg)` exists for the converter's working state but is NOT the mandatory step — the JSON helper is.
- `lm_head` materialization with fused `(1 + g_final)` + rotation. The `RmsNormFusionTarget` `model.language_model.norm.weight → lm_head.weight` returned by `quant::quarot::lm_head::qwen35_final_norm_fusion_target()` composes with `pipeline::fuse_rmsnorms` to fold `(1 + g_final)` into the materialized matrix as a column multiply. Subsequent `absorb_rotations` then input-side absorbs the residual rotation via the pre-existing `opt("lm_head.weight", r_in)` rule in `RotationPlan`. Final-norm fusion applies to **both** tied and untied configs — untied does not get a free pass because `D` still does not commute with `R`.
- Forward-equivalence assertion on a small batch: `‖rotated_forward − original_forward‖ < 1e-5`. Refuse-on-fail (no partial conversion artifacts written). Step 3c-4 landed in PR #28 (`crate::quant::quarot::forward_equivalence::assert_forward_equivalence_qwen35`): the converter binary must call this gate AFTER `materialize_lm_head_for_qwen35` + `fuse_rmsnorms(per_layer + final_norm_target)` + `absorb_rotations` and BEFORE quantizing or writing any output file. The gate runs a deterministic _rotation-chain probe_ — a linearized forward that exercises every linear tensor in the rotation plan (embed_tokens, q_proj/o_proj for GQA, in_proj_z/out_proj for GDN, gate_proj/up_proj/down_proj, lm_head) plus every shifted RMSNorm site. `silu(gate) ⊙ up` is replaced with `gate + up` so the probe is purely linear in `h`; the rotation absorption identities then hold exactly across the chain, so any divergence is a pipeline bug rather than a non-linearity artifact. The probe IS sufficient to catch missing rotation absorption, RMSNorm fusion left online, `lm_head` not materialized for tied configs, `final_norm` fusion target not appended, and wrong-rotation regressions. The probe is NOT sufficient for bugs that only manifest in `silu(gate) ⊙ up` or in the full attention compute — step 4 (perplexity bench on calibration data) is the full check. Refuse semantic is `Err(InferenceError::Inference)` with `max_abs_error`, `mean_abs_error`, `tolerance`, and `probe_tokens` in the message so operators can triage from logs without re-running.
- Q4 bridge — **decision: path (b)**, landed in step 3c-1. Sibling entry points `quantize_f32_to_q4` and `quantize_f64_to_q4` sit alongside the existing `quantize_bf16_to_q4` (all in `crates/inference/src/weights/q4_weights.rs`). Both entry points quantize at **f32 precision** — matching §Risks above ("keep rotation math in f64 [...] quantize in f32, store scales in f16 as before"). The f64 wrapper exists to avoid the BF16 round-trip in `quantize_bf16_to_q4` and to skip an intermediate `Vec<f32>` allocation at the call site; it is NOT a true f64 quantizer (`abs_max`, the scale reciprocal, and the per-nibble `round` all execute in f32). Values within ~½ ULP of an f32 representation may quantize to a different nibble than a hypothetical f64 reference would. Acceptable per §Risks — what matters is that we no longer truncate the rotation pass's output to 7 mantissa bits before quantizing. Measurement (`weights::q4_weights::tests::quantize_f32_to_q4_lower_error_than_bf16_path_on_high_precision_input`, 2048 synthetic f32 values uniform on `[-1, 1]`): path (b) mean abs err `0.034279`, max `0.071060`; path (a) f32→BF16→Q4 mean `0.034312`, max `0.071323`. Path (b) is strictly equal-or-better on max and strictly less on mean across the suite. Delta is small on uniform-rotated inputs but grows for inputs with concentrated absmax (a few outlier values whose BF16 rounding shifts the block scale for all 32 elements).
