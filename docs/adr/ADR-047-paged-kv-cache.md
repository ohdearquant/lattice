...
| LoRA adapter id not propagated to cache call site | Medium — call sites span generation backends       | `PrefixPageCache::lookup` signature requires `AdapterId`; omitting it is a compile error |
