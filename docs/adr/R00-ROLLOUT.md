...
This resolves two parts of R03's PAIR cell. The cell text is kept as quoted in #1596, except for a pointer to this section.

**Prefill mode is not a varied dimension for the R03 checkpoint.** `force_serial_prefill()` (`crates/inference/src/model/qwen35/generation.rs`) is a constant `false` in every non-test build, and only `cfg(test)` code can set it. The Qwen CPU session's prefill (`crates/inference/src/decoder/qwen_cpu.rs`) therefore tries batched prefill first. It falls back to serial prefill only when the batched call refuses a model it does not support, which applies to MoE configurations. For the dense `Qwen3.5-0.8B` acceptance checkpoint, every shipped request takes batched prefill. No request, configuration or seed selects the prefill mode, so there is nothing to pair across. The dimension becomes applicable, and R03's pair must be re-run across it, if either of these changes:

- a non-test build gains a way to select serial prefill for a dense model;
- the row's acceptance checkpoint becomes one whose shipped path is serial prefill.

**The allocation-instrument rules belong to R01.** ADR-090's allocation paragraph describes the instrument that R01 builds by extending the `inference_perf` counting allocator. That paragraph is the one requiring allocation calls, reallocation calls and requested bytes to be counted separately around the warm decode/select/policy region, with a retained-allocation control that must fail. R01 (#1594) owns that instrument and its controls.

R03 carries the routing change and the allocation delta its cell names. That delta is measured with `crates/inference/tests/decoder_allocation_arm.rs` over whole `generate` and `generate_streaming` calls, from base `8952beea` to the head that closes the row. The arm counts allocation and reallocation calls together and does not bracket the warm per-token region, so it does not certify ADR-090's incremental per-token allocation gate for the Qwen CPU route. That certification moves to R01, and nothing waives it. R01 does not close until its instrument has run on the Qwen CPU route with the retained-allocation control failing and the warmed control passing.
