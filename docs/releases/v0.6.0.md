...
- The legacy generic Qwen text-generation module and
  `Qwen35Model::generate_with_batch_prefill` are marked `#[deprecated(since = "0.5.1")]`.
  Originally scheduled for removal in 0.6.0 (#865), the removal target moved to
  **0.7.0** to give downstream consumers a full extra release to migrate before this
  release also landed unrelated breaking changes. Both paths were unused by any in-repo
  binary, example, or library caller and duplicated the canonical decode loop
  (`Qwen35Model::generate` / `generate_streaming`). No code was removed and no behavior
  changed in this release. `lattice-embed`'s `QwenModel::encode` was unaffected.
