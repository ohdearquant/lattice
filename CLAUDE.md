...
`scripts/bench-compare.sh` now enforces the machine side of this itself, so do not wrap it in an external bench-window helper. It takes both machine-wide advisory locks (the bench window and the Metal GPU lock) unconditionally for the whole run. Every run uses ABBA order (`base₁ → head₁ → head₂ → base₂`); report-only controls whether the verdict is propagated, not which evidence is collected. At each of the three arm boundaries — before the first arm, between the measurement-order strata, and after the final arm — macOS runs hold a mandatory 30-second cooldown, then require AC power, nominal thermal state, at least 30 seconds of HID idle, and the portable ambient-CPU idle floor, refusing (in every enforcement mode) to certify a run that fell below any of these at any boundary. Linux CI records that the macOS-only probes are unavailable and still applies the CPU-idle gate. The CPU floor is settable with `BENCH_IDLE_FLOOR`; if you move it, say so wherever the numbers are quoted. Every run prints a `Run conditions` block recording the refs, the effective bench targets and features, the resolution, the arm order, the lock dispositions, the machine-state checkpoints, and the measured idle samples — quote that block along with the numbers, because a figure that does not record what produced it is indistinguishable from one produced on a quiet machine.
...
The three checkpoints above sample the machine at phase _boundaries_ only, so a load that starts and ends between two of them is invisible to all three by construction — this happened twice (a 23-second compile inside one stratum, a 42-second burst that ended 7 seconds before a boundary sample), and both times every boundary sample still read the machine as quiet (lattice#1515). Each of the four ABBA arms (base₁, head₁, head₂, base₂) is therefore also sampled IN-PHASE, at a `PHASE_SAMPLE_INTERVAL`-second cadence (default 5s) for the whole arm, not just at its edges. Each sample splits CPU load into self (the bench's own process tree) and foreign (everything else), since the bench itself is legitimate load that a raw idle floor would misread as noise. Per arm, the report's `Run conditions` block gains an `In-phase load` line — sample count, machine idle min/mean, foreign-load max/mean, self-load mean, and the top foreign process at the foreign-max sample — and `bench-run-provenance.txt` gains a matching `phase_load=` line. If any arm's foreign load exceeds the same `BENCH_IDLE_FLOOR`-derived ceiling the boundary probes use, or if an arm's sampler produced no samples at all, the run refuses (exit 2) rather than being certified. The three boundary checkpoints above are unchanged; in-phase sampling supplements them, it does not replace them.
