...
`scripts/bench-compare.sh` now enforces the machine side of this itself, so do not wrap it in an external bench-window helper. It takes both machine-wide advisory locks (the bench window and the Metal GPU lock) unconditionally for the whole run, samples ambient CPU idle before the first arm, between the measurement-order strata, and after the final arm, and refuses to certify a run that fell below the idle floor at any of them. Enforcing runs use ABBA order (`base₁ → head₁ → head₂ → base₂`); report-only runs retain the faster two-arm comparison. The floor is settable with `BENCH_IDLE_FLOOR`; if you move it, say so wherever the numbers are quoted. Every run prints a `Run conditions` block recording the refs, the effective bench targets and features, the resolution, the arm order, the lock dispositions, and the measured idle samples — quote that block along with the numbers, because a figure that does not record what produced it is indistinguishable from one produced on a quiet machine.
