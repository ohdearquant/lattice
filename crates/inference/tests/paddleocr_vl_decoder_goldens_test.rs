...
fn model_dir() -> Option<PathBuf> {
    let dir = match std::env::var_os("LATTICE_POCR_MODEL_DIR") {
        ...
        None => PathBuf::from(std::env::var_os("HOME")?).join(".lattice/models/paddleocr-vl-1.6"),
    };
...
fn run_gate() {
    ...
    let mut source = SafetensorsFile::open(&dir.join("model.safetensors")).expect("open weights");
