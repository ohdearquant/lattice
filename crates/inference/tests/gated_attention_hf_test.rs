...
//!
//! Previously this test loaded binary fixture files generated by a Python
//! reference script (`dev/hf_reference/gated_attention_ref.py`). Those files
//! are `.bin` and are gitignored, which caused the test to silently skip on
//! clean checkouts.
//!
//! This rewrite generates the reference data deterministically in Rust using
//! the same xorshift64 PRNG and the same exact `f32::exp` that the Python
//! script used. No file I/O, no external dependencies.
//!
//! Run:
...
/// Verify `apply_sigmoid_gate` (dispatched — may use fast-exp SIMD) matches
/// the exact reference within 2e-2.
///
/// The production path uses the Schraudolph fast-exp bit trick (~5-6% relative
/// error on exp), which translates to ~1% absolute error on sigmoid. Tolerance
/// is set to 2e-2 (2%) to cover the full approximation budget.
///
/// This test exercises the production code path that was previously untested by
/// the HF integration test (which only called the scalar path).
#[test]
fn test_sigmoid_gate_dispatched_matches_reference() {
    let q_and_gate = det_data(2 * Q_DIM, SEED_Q_AND_GATE);
    let (_ref_q_buf, ref_gate_buf) = ref_deinterleave(&q_and_gate, NUM_HEADS, HEAD_DIM);
    let context_in = det_data(Q_DIM, SEED_CONTEXT_IN);
    let ref_context_out = ref_apply_sigmoid_gate(&context_in, &ref_gate_buf);
    ...
    assert!(
        diff <= 2e-2,
        "apply_sigmoid_gate (dispatched): max_abs_diff={diff} exceeds 2e-2"
    );
}

/// Verify the dispatched sigmoid path stays within 2% of exact over a realistic
/// gate range [-5, 5] where the Schraudolph approximation has the most impact.
///
/// This is the range encountered in practice for Qwen3.5 gate activations.
#[test]
fn test_sigmoid_gate_dispatched_realistic_range() {
    // 32 uniformly-spaced gate values from -5 to +5.
    let n = 32usize;
    ...
    let mut dispatched = context_in.clone();
    apply_sigmoid_gate(&mut dispatched, &gate);

    let diff = max_abs_diff(&dispatched, &ref_out);
    assert!(
        diff <= 2e-2,
...
/// End-to-end test: deinterleave then apply gate (dispatched path).
///
/// Chains both operations as the model code does. Uses the exact reference for
/// the expected output and tolerates the fast-exp approximation budget.
#[test]
fn test_full_gated_pipeline_matches_reference() {
    let q_and_gate = det_data(2 * Q_DIM, SEED_Q_AND_GATE);
    let context_in = det_data(Q_DIM, SEED_CONTEXT_IN);

    // Reference: exact deinterleave + exact sigmoid.
    let (_ref_q_buf, ref_gate_buf) = ref_deinterleave(&q_and_gate, NUM_HEADS, HEAD_DIM);
    let ref_context_out = ref_apply_sigmoid_gate(&context_in, &ref_gate_buf);

    // Rust: deinterleave + dispatched sigmoid.
    let mut q_buf = vec![0.0f32; Q_DIM];
    let mut gate_buf = vec![0.0f32; Q_DIM];
    ...
    assert!(
        diff <= 2e-2,
        "full pipeline: max_abs_diff={diff} exceeds 2e-2"
    );
}
