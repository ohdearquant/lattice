...
/// **Unstable**: ViT geometry for the vision-language checkpoint variants, parsed from the
/// top-level `vision_config` object in `config.json`. `None` on [`Qwen35Config`] for text-only
/// checkpoints (e.g. the bare 2B/35B-A3B text decoders).
///
/// Parsed but not yet consumed (ADR-069 stage S1) — the Metal ViT forward pass, patch merger,
/// and image-token injection land in later stages.
...
pub struct VisionConfig {
    ...
    /// Temporal patch size (frames per patch) for video input.
...
pub struct Qwen35Config {
    ...
    /// ViT geometry from the top-level `vision_config` object; `None` for text-only checkpoints.
    /// Parsed but not yet consumed by the forward pass (S3/S5 wire it in).
    #[serde(default)]
...
impl Qwen35Config {
    ...
    pub fn qwen35_2b() -> Self {
        ...
        Self {
            ...
            // Vision-language extension (this preset is the bare text decoder, no vision tower)
            vision_config: None,
            image_token_id: None,
            video_token_id: None,
    ...
    /// released checkpoint is a vision-language model; this preset is its bare text decoder
    /// (no vision_config) — parse the real config.json via `from_config_json` to get the
    /// vision-language fields (ADR-069 S1).
...
mod tests {
    ...
    fn test_qwen35_0_8b_config_json_fixture_parses() {
        ...
        // resolve to None (0.8B has no MoE), while the vision_config / token-id fields below
        // are now parsed onto the config (ADR-069 S1) instead of being dropped.
        assert!(!cfg.is_moe(), "Qwen3.5-0.8B is dense, not MoE");
    ...
    fn test_text_only_config_has_no_vision_fields() {
        ...
        // the ADR-069 S1 additions don't regress the plain-text decoder path.
        let json = r#"{
            "text_config": {
                "hidden_size": 1024,
                "num_hidden_layers": 4,
        ...
        // No rope_parameters object at all in this fixture (rope_theta/partial_rotary_factor
        // are flat), so mrope_section has nothing to nest under either.
        assert!(cfg.rope_parameters.is_none());
    }
