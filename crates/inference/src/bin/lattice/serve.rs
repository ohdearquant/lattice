...
pub(super) async fn run(
    ...
    let format = crate::backend::detect_format(model_path);

    eprintln!("Loading model from {model}...");
    let model_backend: ModelBackend = match format {
        crate::backend::ModelFormat::Safetensors => {
            match lattice_inference::model::qwen35::Qwen35Model::from_safetensors(model_path) {
                Ok(m) => ModelBackend::Cpu(Arc::new(m)),
                Err(e) => {
        ...
        crate::backend::ModelFormat::Q4 => {
            #[cfg(feature = "metal-gpu")]
            {
                let tokenizer_dir_path = tokenizer_dir.as_ref().map(std::path::PathBuf::from);
                match ModelBackend::spawn_metal(
            ...
            #[cfg(not(feature = "metal-gpu"))]
            {
                ...
                eprintln!(
                    "Error: {}",
                    crate::backend::metal_gpu_required_message(model_path)
                );
                std::process::exit(1);
            }
        }
        crate::backend::ModelFormat::Unknown => {
            eprintln!(
                "Error: {}",
                crate::backend::unrecognized_format_message(model_path)
            );
            std::process::exit(1);
        }
    };
    ...
    eprintln!("Listening on {addr}  (model: {served_model_id}, max_tokens default: {max_tokens})");
...
impl ModelBackend {
    ...
    pub fn spawn_metal(
        ...
        let (vision_config, max_context) = crate::chat::load_q4_serve_config(&model_dir)?;
        ...
        let (owner, client, _meta) = MetalWorker::spawn_with_vision(
            move || {
                let (cfg, _) = crate::chat::load_q4_serve_config(&model_dir_for_loader)?;
                let state =
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Streaming context-overflow status parity (ADR-080 C2): `lattice/serve.rs` already gets this right
    ...
    mod production_adapter_observation {
        ...
        use lattice_inference::serve::{
            ...
            PRODUCTION_ADAPTER_PARITY_CASES, PRODUCTION_ADAPTER_PARITY_CONTEXT_WINDOW,
            ProductionAdapterObservation, ProductionAdapterParityCase,
            ProductionAdapterParityOutcome, assert_observation_matches,
        };
        use std::sync::Mutex;
        use tower::ServiceExt as _;
        ...
        async fn run_observed(case: &ProductionAdapterParityCase, stopped: bool) -> CaseRun {
            ...
            let bytes = axum::body::to_bytes(response.into_body(), usize::MAX)
                .await
                .expect("response body must be readable");
            let body = serde_json::from_slice(&bytes).expect("response body must be valid JSON");
