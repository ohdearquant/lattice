...
pub fn normalize_request<C>(
    ...
    normalize_request_with_message_normalizer(
        req,
        defaults,
        profile,
        normalize_messages,
...
/// Normalize one shared wire request with a server-specific message converter.
///
/// This preserves the shared validation cascade while allowing a
/// vision-capable server to decode image content parts into engine messages.
pub fn normalize_request_with_message_normalizer<C>(
    req: &ChatRequest,
    defaults: GenerationDefaults,
    profile: ServeProfile<'_>,
    normalize: impl FnOnce(&[Message]) -> Result<Vec<ChatMessage>, ApiError>,
    check_context: impl FnOnce(&[ChatMessage], usize) -> Result<C, ApiError>,
) -> Result<(ValidatedChatRequest, C), ApiError> {
    ...
    let messages = normalize(&req.messages)?;
