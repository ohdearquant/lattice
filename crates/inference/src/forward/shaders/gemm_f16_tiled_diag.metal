...
// #535 control 2 diagnostic only: byte-for-byte the same BM/BN/BK tiling, X
// staging, threadgroup layout, and simdgroup MMA accumulation order as
// gemm_q8_tiled (see gemm_q8_tiled.metal), but QW is a flat row-major f16
// [N,K] array instead of Q8_0 blocks (2-byte scale + 32 int8 per 32 values).
// The dequantization step is replaced by a direct half load. This isolates
// tiled-GEMM staging/accumulation order from the Q8_0 representation itself.
kernel void gemm_f16_tiled_diag(
    device const half* QW  [[buffer(0)]],
    ...
    uint sg                [[simdgroup_index_in_threadgroup]])
{
    ...
    for (uint k0 = 0; k0 < K; k0 += BK) {
        ...
        for (uint i = tid; i < BN*BK; i += THREADS) {
            uint ni=i/BK; uint ki=i%BK; uint gn=n0+ni; uint gk=k0+ki;
            Wtg[ki][ni] = (gn<N) ? QW[gn*K+gk] : half(0.0f); }
