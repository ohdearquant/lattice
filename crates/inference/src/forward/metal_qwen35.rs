...
// Gated behind LATTICE_GDN_CHUNKED=1; 0.8B shape only (kd=vd=128, 16 heads, ratio=1).
// Serial path (gdn_recurrence_fused) is unchanged default.
...
"#;
    ...
    struct MetalQwen35Activations {
        ...
        gdn_chunk: GdnChunkScratch, // chunked-scan scratch (LATTICE_GDN_CHUNKED=1 only)
    ...
    impl MetalQwen35State {
        ...
        fn forward_prefill_batched_chunk(
            ...
            if chunked_requested && !chunked_supported {
                tracing::warn!(
                    "LATTICE_GDN_CHUNKED=1 ignored: chunked GDN prefill supports only Qwen3.5-0.8B"
                );
            }
        ...
        fn encode_gdn_layer(
            ...
            unsafe {
                ...
                // LATTICE_GDN_CHUNKED=1 is prefill-only; single-token decode always uses serial path.
                if Self::gdn_chunked_prefill_requested() && !use_q36 {
                    tracing::debug!(
                        "LATTICE_GDN_CHUNKED=1 is prefill-only; encode_gdn_layer keeps serial recurrence"
                    );
                }
        ...
        // -----------------------------------------------------------------------
        // GDN chunked-scan helpers (LATTICE_GDN_CHUNKED=1)
        // -----------------------------------------------------------------------

        fn gdn_chunked_prefill_requested() -> bool {
            std::env::var_os("LATTICE_GDN_CHUNKED").is_some()
        }
...
"#;
        ...
        /// Gate 2: B-vs-B self-consistency.  Running forward_prefill (chunked path, LATTICE_GDN_CHUNKED=1)
        ...
        fn gdn_chunked_b_vs_b_self_consistency() {
            if std::env::var("LATTICE_GDN_CHUNKED").as_deref() != Ok("1") {
                eprintln!(
                    "skipping gdn_chunked_b_vs_b_self_consistency: set LATTICE_GDN_CHUNKED=1"
                );
                return;
            }
            let model_dir =
        ...
        fn gdn_chunked_state_vs_serial_state_diff() {
            if std::env::var("LATTICE_GDN_CHUNKED").as_deref() != Ok("1") {
                eprintln!(
                    "skipping gdn_chunked_state_vs_serial_state_diff: set LATTICE_GDN_CHUNKED=1"
                );
                return;
            }
            let model_dir =
            ...
            for attempt in 0..3 {
                ...
                unsafe { std::env::set_var("LATTICE_GDN_CHUNKED", "1") };
        ...
        fn gdn_chunked_race_localization_probe() {
            if std::env::var("LATTICE_GDN_CHUNKED").as_deref() != Ok("1") {
                eprintln!("skipping race-localization probe: set LATTICE_GDN_CHUNKED=1");
                return;
            }
            let model_dir =
                std::path::PathBuf::from(std::env::var("HOME").unwrap_or_else(|_| ".".into()))
