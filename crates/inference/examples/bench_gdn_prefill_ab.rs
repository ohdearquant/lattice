/// Benchmark: Qwen3.5-0.8B Metal `forward_prefill` latency for serial vs
/// `LATTICE_GDN_CHUNKED=1` GDN prefill.
///
/// Usage:
...
fn run() {
    ...
    let mode = if std::env::var_os("LATTICE_GDN_CHUNKED").is_some() {
        "chunked"
    } else {
        "serial"
    };
...
#[cfg(all(target_os = "macos", feature = "metal-gpu"))]
fn set_chunked(enabled: bool) {
    // This benchmark is single-threaded and mutates the process environment only
    // around immediate calls into `forward_prefill`, whose gate reads this flag.
    unsafe {
        if enabled {
            std::env::set_var("LATTICE_GDN_CHUNKED", "1");
        } else {
            std::env::remove_var("LATTICE_GDN_CHUNKED");
        }
    }
}
