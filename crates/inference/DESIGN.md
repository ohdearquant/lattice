...
**Embedding cache.** `Mutex<HashMap<u64, Vec<f32>>>` at `qwen.rs:391`. Key = `hash_token_ids(&ids) -> u64`. Cap: 10K entries (~40MB at 1024d). Eviction: flush-all at capacity (`qwen.rs:723-725`) -- simple but effective; embeddings are deterministic, so cache misses just recompute. LRU would add complexity for minimal benefit given mostly-unique-text workloads. Persistence: fail-closed manifest format `[magic:8][manifest_len:u32 LE][manifest JSON][payload]`, where the payload is the record stream `[hash:u64, dim:u32, floats:f32*dim]` per entry and the manifest carries `version`, `payload_sha256`, `base_model_rev`, `tokenizer_rev`, `embedding_dim`, and `entry_count`. On load the parser rejects (returns `Err`, never load-and-warn) on magic/version/SHA-256/base-model/tokenizer/dimension/finiteness/trailing-byte mismatch; legacy raw record-stream files (no magic) are rejected, not migrated (#504 first slice). Cache hit returns in <1us vs ~100ms forward pass.
